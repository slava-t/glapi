...
class GitlabApi {
  ...
  getChildPipelineIds(pipelineIds, name) {
    return Promise.all(pipelineIds.map(x => this.getChildPipelineId(x, name)));
  }
  ...
  async createPipeline(branch, vars) {
    ...
    try {
      ...
      this._logInfo(
        `pipeline vars: ${JSON.stringify(pipelineVars, null, 2)}`
      );
