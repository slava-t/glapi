...
const DEFAULT_BRIDGE_TIMEOUT = 15 * 60 * 1000;
...
class GitlabApi {
  ...
  async pipelineExists(id) {
    const uri = `${this._pipelinesUri}/${id}`;
    this._logRequest('pipelineExists requester.get', uri);
    const res = await this._requester.get(uri);
    if (res.status >= 200 && res.status < 300) {
      return true;
    }
    ...
    throwIfErrorResult(
      res,
      `Checking pipeline ${id} existence failed`,
      this._logger
    );
  }
  ...
  async tryCancelPipelines() {
    ...
    for (const pipelineId of args) {
      try {
        const exists = await this.pipelineExists(pipelineId);
        if (!exists) {
          this._logWarn(`Pipeline ${pipelineId} does not exist, skipping cancel`);
          continue;
        }
  ...
  async _getLastJobFinishedDate(pipelineId) {
    const jobs = await this.getPipelineJobs(pipelineId);
    if (jobs.length < 1) {
      return;
    }
    const lastJob = jobs[jobs.length - 1];
    const lastJobFinishedDateStr = lastJob['finished_at'];
    if (lastJobFinishedDateStr) {
      return new Date(lastJobFinishedDateStr);
    }
  }

  async _pipelineBridgeStatus(baseTime, pipelineId) {
    const now = this._clock.now();
    const bridges = await this.getPipelineBridges(pipelineId);
    for (const bridge of bridges) {
      ...
      if (!child && now - baseTime > DEFAULT_BRIDGE_TIMEOUT) {
        return 'timeout';
      }
    }
    return 'wait';
  ...
  async waitForPipeline(
    ...
    let lastJobFinishedDate;
    let pipelineBridgeStatus = 'wait';
    while (true) {
      try {
        ...
        const now = this._clock.now();
        if (!lastJobFinishedDate) {
          lastJobFinishedDate = await this._getLastJobFinishedDate(pipelineId);
        }
        if (lastJobFinishedDate) {
          if (pipelineBridgeStatus == 'wait') {
            pipelineBridgeStatus = await this._pipelineBridgeStatus(
              lastJobFinishedDate.getTime(),
              pipelineId
            );
          }
          if (pipelineBridgeStatus == 'timeout') {
            this._logError(
              `Bridge creation timeout detected for pipeline ${pipelineId}`
            );
            this._logInfo(
              `Canceling pipeline ${pipeline['web_url']}`
            );
            await this.tryCancelPipelines(pipelineId);
            pipeline.status = 'bridge_timeout';
            return pipeline;
          }
        }
        if (now - startTime > timeout) {
