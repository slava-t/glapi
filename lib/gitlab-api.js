...
const {throwIfErrorResult} = require('./util.js');
...
const pipeline_waiting_statuses = new Set([
  'created',
...
const isPipelineWaiting = function(pipeline) {
  return pipeline_waiting_statuses.has(pipeline.status);
};
...
class GitlabApi {
    ...
    try {
      ...
      if (!this._projectId) {
        if (this._projectPath) {
          const project = await getProjectByPath(this._projectPath);
          if (!project) {
  ...
  async createPipelines(branch, count = 1) {
    const result = [];
    for (let i = 0; i < count; ++i) {
      try {
        const pipeline = await this.createPipeline(branch);
  ...
  async waitForPipeline(pipeline, startTime, timeout = DEFAULT_PIPELINE_TIMEOUT) {
    ...
    while (true) {
      ...
      try {
        ...
        this._logInfo(`status for pipeline ${pipelineId} is ${res.data.status}`);
  ...
  async createBatch(branch, batchSize, pipelineTimeoutInMinutes) {
    ...
    const pipelines = await this.createPipelines(branch, batchSize);
    this._logInfo(
